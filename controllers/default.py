...
def wait_tsfa():
    ...
    if scheduler.task_status(request.args[0]).status=="COMPLETED":
        ...
        while(seq!=""):
            ...
            col,nmap,domain_db,count_annotation,root_second_level,controls=fishertest.load(request.args[1],seq[0],seq[1:]+proteome_annotated[seq[0]],
            ...
            fisher_h[seq[0]],controls_h=fisher_hierarchy.load(request.args[1],seq[1:]+proteome_annotated[seq[0]],float(request.args[2]),db_hierarchy,request.folder,
...
def load_bar_plot():
    path="/magneto/static/results/528e81e8-2bf0-4da7-bc5b-f2c31dfe7eaa/0_bar.txt"
    return dict(path=path)
def load_chord():
